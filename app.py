...
def load_data():
    ...
    if not os.path.exists(file_path):
        st.error(f"File not found: {file_path}. Upload gas_prices.csv to GitHub in the same folder as app.py.")
        st.stop()
    ...
    # Map whatever your CSV uses -> to 'date' and 'price'
    if 'dates' in df.columns and 'prices' in df.columns: # <-- YOUR CSV CASE
        df = df.rename(columns={'dates': 'date', 'prices': 'price'})
    elif 'observation date' in df.columns and 'value' in df.columns:
        df = df.rename(columns={'observation date': 'date', 'value': 'price'})
    elif 'date' in df.columns and 'price' in df.columns:
        pass # already correct
    elif 'date' in df.columns and 'prices' in df.columns:
        df = df.rename(columns={'prices': 'price'})
    else:
        st.error(f"CSV must have Date+Price columns. Found: {df.columns.tolist()}")
        st.stop()
...
def price_storage_contract(injection_dates, withdrawal_dates, prices_df, 
    ...
    Constraints: rate, max_volume, storage_cost. Interest=0, no delay per JPM task.
    """
    ...
    for date, action in events:
        ...
        if action == 'inject':
            amt = min(rate, max_volume - current_volume) # can't exceed capacity
            cashflow -= amt * price
            current_volume += amt
            log.append([date, 'Inject', amt, price, -amt*price, current_volume])
            
        elif action == 'withdraw':
            amt = min(rate, current_volume) # can't go negative
            cashflow += amt * price
            current_volume -= amt
            log.append([date, 'Withdraw', amt, price, amt*price, current_volume])

    # Storage cost: charge $/Bcf/day for all gas held
    if len(events) > 1:
...
st.write(f"Loaded {len(df)} rows of gas data: {df['date'].min().date()} to {df['date'].max().date()}")

col1, col2 = st.columns(2)
with col1:
    inject_dates_str = st.text_area("Injection Dates YYYY-MM-DD, comma separated", "2020-01-02")
    rate = st.number_input("Max Injection/Withdrawal Rate Bcf/day", min_value=1, value=100)
    max_vol = st.number_input("Max Storage Volume Bcf", min_value=1, value=1000)
with col2:
    withdraw_dates_str = st.text_area("Withdrawal Dates YYYY-MM-DD, comma separated", "2020-12-01")
...
if st.button("Price Contract"):
    ...
    st.line_chart(df.set_index('date')['price'], height=200)
